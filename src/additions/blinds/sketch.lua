SMODS.Atlas({
	key = "twbl_blind_atlas_sketch",
	px = 34,
	py = 34,
	path = "blinds/sketch.png",
...
TW_BL.blinds.bootstrap_interactive_blind(blind, {
	connected_status_text = function()
		return localize("k_twbl_target_ex")
	end,
	command = "target",
	command_max_uses = 1,
	cards_voting = true,
	...
	on_new_provider_command = function(event, args, card)
		...
		then
			TW_BL.utils.vote_for_card(card, event, G.C.CHIPS)
			return true
		end
	end,
		...
		G.E_MANAGER:add_event(Event({
			func = function()
				card_eval_status_text(
					...
					nil,
					nil,
					{ message = localize("k_twbl_sketch_ex"), colour = G.C.ORANGE }
				)
