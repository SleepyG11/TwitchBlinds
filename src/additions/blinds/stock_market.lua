...
TW_BL.blinds.bootstrap_interactive_blind(blind, {
	...
	on_new_provider_command = function(event, args, card)
		if
			card
			and card.set_cost
			and to_big(G.GAME.dollars) > to_big(G.GAME.bankrupt_at - 20)
		...
		then
            local amount_to_reinvest = math.max(math.ceil(G.GAME.dollars / 50), 1)
			ease_dollars(-1 * amount_to_reinvest, true)
			card.ability.extra_value = (card.ability.extra_value or 0) + amount_to_reinvest
			card:set_cost()
