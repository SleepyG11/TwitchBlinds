...
function tw_sticker:__naturally_apply(card, center, area, check_for_tag)
	if check_for_tag and not card.from_tag then
		return
	end
	if tw_sticker:should_apply(card, center, area or card.area) then
...
function tw_sticker:__get_pool(kind)
	local result_pool = {}
	local pool_to_copy = {}
...
function tw_sticker:__emplace_cards(kind)
	...
	elseif kind == "Celestial" then
		local pool = tw_sticker:__get_pool("Celestial")
		for i = 1, 5 do
...
function tw_sticker:__use()
	...
	local kind = TW_BL.G.state_sticker_chat_booster
	...
	elseif kind == "Arcana" or kind == "Spectral" then
		...
		G.E_MANAGER:add_event(Event({
			func = function()
				local card_to_use = G.twbl_chat_booster_area and G.twbl_chat_booster_area.cards[1]
...
function tw_sticker:__on_booster_open(card)
	...
	TW_BL.G.state_sticker_chat_booster = kind
	...
	if kind == "Spectral" or kind == "Arcana" then
		...
		tw_sticker:__emplace_cards(kind)
...
TW_BL.EVENTS.add_listener("twitch_command", "twbl_sticker_chat_booster", function(command, username, raw_index)
	...
	if kind == "Celestial" or kind == "Standard" then
		if
			index
			and G.twbl_chat_booster_area
			and G.twbl_chat_booster_area.cards
			and G.twbl_chat_booster_area.cards[index]
		then
			TW_BL.CHAT_COMMANDS.increment_command_use(command, username)
			local card = G.twbl_chat_booster_area.cards[index]
			...
			card_eval_status_text(
				...
				nil,
				nil,
				{ message = username, colour = kind == "Standard" and G.C.MONEY or G.C.CHIPS, instant = true }
			)
		end
	elseif kind == "Arcana" or kind == "Spectral" then
		if index and G.hand and G.hand.cards and G.hand.cards[index] then
			...
			card_eval_status_text(
				...
				nil,
				nil,
				{ message = username, colour = kind == "Arcana" and G.C.PURPLE or G.C.CHIPS, instant = true }
			)
		end
	end
end)
